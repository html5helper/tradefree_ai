...
class OSSUploader:
    ...
    def __init__(
        ...
        if not access_key_id or not access_key_secret:
            raise ValueError(
                "Access Key ID and Secret must be set as environment variables"
            )
        ...
        self.host = os.getenv("OSS_HOST") or "http://image.tradefree.shop"
    ...
    def upload_file_with_url(
        ...
        return {"oss_path": oss_path, "http_url": http_url}

    def upload_bytes(
        self, file_content: bytes, oss_file_path: str, headers: Optional[dict] = None
    ) -> str:
        """
        上传二进制内容到OSS
