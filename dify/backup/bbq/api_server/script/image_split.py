...
try:
    ...
    print(
        "警告: rembg库未安装，背景移除功能将不可用。请运行 'pip install rembg' 安装。"
    )


def remove_background(img, output_path=None, log_callback=None):
    """移除图片背景，确保输出为PNG格式

    Args:
        img (PIL.Image or str): 输入图片对象或图片文件路径
        output_path (str, optional): 输出图片路径，如果提供则保存为PNG
    ...
    if not REMBG_AVAILABLE:
        if log_callback:
            log_callback(
                "警告: rembg库未安装，无法移除背景。请运行 'pip install rembg' 安装。"
            )
        return img
    ...
    try:
        ...
        # 使用rembg移除背景（rembg会自动处理为带透明通道的格式）
        ...
        if output_path:
            ...
            if os.path.splitext(output_path)[1].lower() != ".png":
                ...
                if log_callback:
                    log_callback(f"输出路径已自动转换为PNG格式: {output_path}")

            # 保存为PNG格式，保留透明通道
            result.save(output_path, "PNG")

            if log_callback:
                log_callback(f"已保存无背景PNG图片到: {output_path}")
...
def split_image(
    image_path, output_dir=None, split_mode="4", remove_bg=True, log_callback=None
):
    """分割单张图片（支持四宫格和九宫格）
    ...
    Args:
        ...
        remove_bg (bool, optional): 是否移除背景，默认为True
        ...
        with Image.open(image_path) as img:
            ...
            if split_mode == "9":
                ...
                if width != height:
                    if log_callback:
                        log_callback(
                            f"警告: {filename_base} 不是正方形图片 ({width}x{height})，九宫格分割需要裁剪为正方形"
                        )
                    # 取较小的尺寸作为正方形边长
                ...
                if width != height and log_callback:
                    log_callback(
                        f"信息: {filename_base} 为长图 ({width}x{height})，四宫格分割将保持完整性"
                    )
            ...
            for i, pos in enumerate(positions, 1):
                ...
                # 如果需要移除背景，对每个子图单独处理背景移除并直接保存
                if remove_bg:
                    if log_callback:
                        log_callback(f"正在移除 {filename_base}_{i} 的背景...")
                    # 直接处理并保存无背景图片
                    remove_background(sub_img, output_path, log_callback)
                else:
                    # 不需要移除背景，直接保存
                    # 保存时保持原格式
